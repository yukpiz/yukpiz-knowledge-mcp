    ...
    Statement = [
      {
        ...
        Resource = [
          "arn:aws:s3vectors:${var.aws_region}:*:vector-bucket/${aws_s3_vector_bucket.this.bucket}",
          "arn:aws:s3vectors:${var.aws_region}:*:vector-bucket/${aws_s3_vector_bucket.this.bucket}/index/*"
        ]
      }
    ]
  })
